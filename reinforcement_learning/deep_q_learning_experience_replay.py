...
def main(
    ...
    for t in range(burnin_period):
        ...
        replay_memory.append(exp.flatten())
        if reward > 0:
    ...
    for t in range(timesteps_per_episode):
        ...
        for j in range(minibatch_size):
            ...
            # TODO why does the network use the successor state as input?
            target_input = np.expand_dims(minibatch_exp.successor_bitarray, axis=0)
            target_output = target_model.predict(target_input)[0]
            best_q_value = np.max(target_output)
            labels[j] = minibatch_exp.reward + gamma * best_q_value
